...
const Modal = () =>{
    ...
    return <MuiModal open={modal} onClose={handleClose} className='fixed !top-7 left-0 right-0 z-50 mx-auto w-full max-w-5xl overflow-hidden overflow-y-scroll scrollbar-hide'>
        <>
                ...
                />
                 <div className='absolute bottom-10 left-10 flex w-full items-center justify-between px-18'>
                     <div className='flex space-x-2'>
                         ...
                         <button className='modalButton' onClick={() =>setMuted(prev => !prev)}>{muted ? <BsVolumeMute className='w-7 h-7'/> : <BsVolumeDown className='w-7 h-7'/>}</button>
                     </div>
                 </div>
